...
export async function v1Routes(app: FastifyInstance) {
  ...
  contract(app, {
    ...
    run: ({ user, params, body }) =>
      tenants.update(user.id, params.organizationId, body.name),
  });
  contract(app, {
    method: "GET",
  ...
  contract(app, {
    ...
    tag: "Multi-tenancy",
    summary:
      "Issue an invitation; the token is returned once for secure sharing",
    params: s.orgParams,
    body: z
      .object({
    ...
    status: 201,
    run: ({ user, params, body }) =>
      tenants.invite(user.id, params.organizationId, body.email, body.role),
  });
  contract(app, {
    method: "POST",
