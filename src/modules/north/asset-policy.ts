...
import type { AssetConfiguration, AssetCategory } from "./asset-config.js";
import { fail } from "../../shared/errors.js";

type Format = { category: AssetCategory; magic: (bytes: Uint8Array) => boolean };
const starts = (expected: number[]) => (bytes: Uint8Array) =>
  bytes.length >= expected.length && expected.every((value, index) => bytes[index] === value);

export const assetFormats: Readonly<Record<string, Format>> = Object.freeze({
...
export function validateAssetDeclaration(input: { filename: string; mime: string; size: number; checksum: string }, config: AssetConfiguration) {
  ...
  if (!format) fail(422, "ASSET_MIME_NOT_ALLOWED", "Asset MIME type is not allowed");
  ...
  if (!/^[0-9a-f]{64}$/.test(checksum)) fail(422, "ASSET_CHECKSUM_INVALID", "Asset checksum must be SHA-256 hex");
  return { filename, mime: input.mime, size: input.size, checksum, category: format.category };
}

export function validateInspectedAsset(
  declared: { mime: string; size: bigint; checksum: string },
  inspected: { mime?: string; size: number; checksum?: string; prefix: Uint8Array },
) {
  if (BigInt(inspected.size) !== declared.size) fail(422, "ASSET_SIZE_MISMATCH", "Uploaded asset size does not match declaration");
  if (inspected.mime !== declared.mime) fail(422, "ASSET_MIME_MISMATCH", "Uploaded asset MIME does not match declaration");
