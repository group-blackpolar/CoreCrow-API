...
function assetView(asset: {
  id: string; organizationId: string; ownerId: string; filename: string; mime: string;
  size: bigint; checksum: string; status: string; confirmedAt: Date | null;
...
export class NorthAssetService {
  ...
  private async scoped(tx: Transaction, userId: string, organizationId: string, assetId: string, capability: "north.asset.read" | "north.asset.create" | "north.asset.delete") {
  ...
  async requestUpload(userId: string, organizationId: string, input: AssetInput) {
  ...
  private async terminal(
    ...
    const asset = await transaction(async (tx) => {
      ...
      if (current.deletedAt) fail(404, "NOT_FOUND", "Asset not found");
  ...
  async confirm(userId: string, organizationId: string, assetId: string) {
    const asset = await transaction(async (tx) => {
      const current = await repo.asset(tx, organizationId, assetId);
      if (!current) fail(404, "NOT_FOUND", "Asset not found");
      await authorizeNorth(tx, userId, "north.asset.create", target(organizationId));
      if (current.deletedAt) fail(404, "NOT_FOUND", "Asset not found");
      if (current.status === "READY") return current;
      if (current.status === "REJECTED" || current.status === "QUARANTINED")
        fail(409, "ASSET_NOT_PROCESSABLE", "Asset is not processable");
      if (current.status === "UPLOADING") return repo.status(tx, current.id, "PROCESSING");
      return current;
    });
    ...
    if (verdict !== "APPROVED") {
      ...
      if (status === "REJECTED") await this.dependencies.storage.delete(asset.storageKey).catch(() => undefined);
  ...
  async signedRead(userId: string, organizationId: string, assetId: string) {
    const asset = await transaction(async (tx) => {
      const current = await this.scoped(tx, userId, organizationId, assetId, "north.asset.read");
      if (current.deletedAt || current.status !== "READY") fail(404, "NOT_FOUND", "Asset not found");
      return current;
    });
  ...
  async validateReference(userId: string, organizationId: string, assetId: string, existingTransaction?: Transaction) {
