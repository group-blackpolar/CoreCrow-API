...
const positiveInteger = (name: string, fallback: number, minimum = 1, maximum = Number.MAX_SAFE_INTEGER) => {
...
export type AssetConfiguration = {
  ...
  defaultOrganizationStorageLimitBytes: number;
  maximumBytes: Record<AssetCategory, number>;
};

export function assetConfiguration(): AssetConfiguration {
  return {
    ...
    defaultOrganizationStorageLimitBytes: positiveInteger(
      "NORTH_ASSET_DEFAULT_STORAGE_LIMIT_BYTES",
      10 * 1024 * 1024 * 1024,
      1024 * 1024,
    ),
    maximumBytes: {
