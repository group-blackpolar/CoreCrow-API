...
export const commerce = {
  ...
  async product(actorId: string, data: Parameters<typeof repo.product>[1]) {
  ...
  async plan(actorId: string, data: Parameters<typeof repo.plan>[1]) {
    await requireOperator(actorId, true);
    return transaction(async (tx) => {
  ...
  async provision(
    actorId: string,
    data: { organizationId: string; planId: string; endsAt: string },
    idempotencyKey: string,
  ) {
    await requireOperator(actorId, true);
    const requestHash = hash(
      JSON.stringify([data.organizationId, data.planId, data.endsAt]),
    );
    return transaction(async (tx) => {
      ...
      const result = await repo.provision(
        tx,
        { ...data, endsAt: new Date(data.endsAt), idempotencyKey, requestHash },
        plan,
      );
      await audit.append(tx, {
        actorId,
  ...
  async cancel(actorId: string, organizationId: string, id: string) {
    await requireOperator(actorId, true);
    return transaction(async (tx) => {
      const subscription = await repo.find(tx, id, organizationId);
      if (!subscription) fail(404, "NOT_FOUND", "Subscription not found");
  ...
  list(actorId: string, organizationId: string) {
    return transaction(async (tx) => {
      await authorize(tx, actorId, organizationId, "commerce.read");
      return repo.list(tx, organizationId);
    });
  },
  entitlements(actorId: string, organizationId: string) {
    return transaction(async (tx) => {
      await authorize(tx, actorId, organizationId, "commerce.read");
