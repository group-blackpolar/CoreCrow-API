export const statusWindows = ["1h", "6h", "24h"] as const;
export type StatusWindow = (typeof statusWindows)[number];

const windowMilliseconds: Record<StatusWindow, number> = {
  "1h": 60 * 60 * 1000,
  "6h": 6 * 60 * 60 * 1000,
  "24h": 24 * 60 * 60 * 1000,
};
...
export class RequestTelemetry {
  ...
  record(durationMs: number, statusCode: number) {
    ...
    const bucket = this.buckets.get(startedAt) ?? {
      ...
      errors4xx: 0,
      errors5xx: 0,
      latencyHistogram: Array.from(
        { length: latencyBounds.length + 1 },
        () => 0,
      ),
    };
    bucket.requests += 1;
    bucket.totalLatencyMs += durationMs;
    if (statusCode >= 400 && statusCode < 500) bucket.errors4xx += 1;
    if (statusCode >= 500) bucket.errors5xx += 1;
    const latencyIndex = latencyBounds.findIndex(
      (bound) => durationMs <= bound,
    );
    bucket.latencyHistogram[
      latencyIndex === -1 ? latencyBounds.length : latencyIndex
    ]! += 1;
  ...
  summary(window: StatusWindow) {
    const now = this.now();
    const requestedStart = now - windowMilliseconds[window];
    const observedStart = Math.max(this.startedAt, requestedStart);
    const firstBucket =
      Math.floor(observedStart / bucketMilliseconds) * bucketMilliseconds;
    const lastBucket =
      Math.floor(now / bucketMilliseconds) * bucketMilliseconds;
    const selected = [...this.buckets.values()].filter(
      (bucket) => bucket.startedAt >= firstBucket,
    );
    const requests = selected.reduce((sum, bucket) => sum + bucket.requests, 0);
    const totalLatencyMs = selected.reduce(
      (sum, bucket) => sum + bucket.totalLatencyMs,
      0,
    );
    const errors4xx = selected.reduce(
      (sum, bucket) => sum + bucket.errors4xx,
      0,
    );
    const errors5xx = selected.reduce(
    ...
    const histogram = Array.from(
      ...
      (_, index) =>
        selected.reduce(
          (sum, bucket) => sum + (bucket.latencyHistogram[index] ?? 0),
          0,
        ),
    );
    const coverageSeconds = Math.max(
      1,
      Math.floor((now - observedStart) / 1000),
    );
    const series = [];
    for (
      let startedAt = firstBucket;
      startedAt <= lastBucket;
      startedAt += bucketMilliseconds
    ) {
      const bucket = this.buckets.get(startedAt);
      series.push({
...
export type StatusSummary = ReturnType<RequestTelemetry["summary"]>;
