...
2. Set `DATABASE_URL`, a strong `BETTER_AUTH_SECRET`, HTTPS `BETTER_AUTH_URL`, exact `TRUSTED_ORIGINS`, `SMTP_URL`, `SMTP_HELO_NAME`, and `MAIL_FROM` on the backend host. `SMTP_HELO_NAME` must be the public FQDN of the sending host (for example, `api.blackpolar.org`), never an ephemeral Docker hostname. Update the Google callback to `/v1/auth/callback/google` if enabled. Do not put secrets into either frontend.
