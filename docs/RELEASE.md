...
4. Build/start the new API, verify `/v1/live`, `/v1/health`, root HTML and a real email verification flow. Readiness is degraded if SMTP configuration is missing; configured SMTP is not a delivery guarantee. Email-dependent requests return 503 before mutation while mail settings are missing.
