...
export async function v1Routes(app) {
    ...
    contract(app, {
        ...
        run: ({ user, body, request }) => {
            ...
            return changeTemporaryPassword(user.id, sessionId, body.currentPassword, body.newPassword);
        },
    });
    contract(app, {
        method: "POST",
    ...
    contract(app, {
        ...
        response: s.org,
        run: ({ user, params, body }) => platform.setOrganizationStatus(user.id, params.id, body.status),
    });
    contract(app, {
        method: "GET",
        ...
        response: z.object({
            ...
            byStatus: z.array(z.object({ status: billingStatus, count: z.number().int().nonnegative() })),
        }),
    ...
    contract(app, {
        ...
        response: s.org,
        run: ({ user, params, body }) => tenants.update(user.id, params.organizationId, body),
    });
    contract(app, {
        method: "PATCH",
        ...
        response: s.org,
        run: ({ user, params, body }) => tenants.setStatus(user.id, params.organizationId, body.status),
    });
    contract(app, {
        method: "GET",
    ...
    contract(app, {
        ...
        body: z
            ...
            .strict(),
        response: billingView,
        run: ({ user, params, body }) => billing.updateEmail(user.id, params.organizationId, body.billingEmail),
    });
    contract(app, {
        method: "PATCH",
    ...
    contract(app, {
        ...
        tag: "Authorization",
        summary: "Resolve deterministic effective permissions from role, groups, and direct grants",
        params: s.orgParams,
        response: z.object({
            role: s.role,
            permissions: z.array(z.enum(permissions)),
        }),
        run: ({ user, params }) => authorizationGroups.effective(user.id, params.organizationId),
    });
    ...
    contract(app, {
        ...
        run: ({ user, params }) => authorizationGroups.list(user.id, params.organizationId),
    });
    ...
    contract(app, {
        ...
        status: 201,
        run: ({ user, params, body }) => authorizationGroups.create(user.id, params.organizationId, body),
    });
    contract(app, {
        method: "PATCH",
        ...
        body: z
            ...
            .refine((value) => Object.keys(value).length > 0),
        response: s.organizationGroup,
        run: ({ user, params, body }) => authorizationGroups.update(user.id, params.organizationId, params.groupId, body),
    });
    contract(app, {
        method: "DELETE",
        url: "/organizations/:organizationId/groups/:groupId",
        ...
        response: z.null(),
        run: async ({ user, params }) => {
            await authorizationGroups.remove(user.id, params.organizationId, params.groupId);
            return null;
        },
    });
    contract(app, {
        ...
        status: 201,
        run: ({ user, params, body }) => authorizationGroups.addMember(user.id, params.organizationId, params.groupId, body.userId),
    });
    contract(app, {
        method: "DELETE",
        ...
        response: z.null(),
        run: async ({ user, params }) => {
            await authorizationGroups.removeMember(user.id, params.organizationId, params.groupId, params.userId);
            return null;
        },
    });
    contract(app, {
        ...
        status: 201,
        run: ({ user, params, body }) => authorizationGroups.grantGroup(user.id, params.organizationId, params.groupId, body.permission),
    });
    contract(app, {
        method: "DELETE",
        ...
        response: z.null(),
        run: async ({ user, params }) => {
            await authorizationGroups.revokeGroup(user.id, params.organizationId, params.groupId, params.permission);
            return null;
        },
    });
    contract(app, {
        ...
        run: ({ user, params }) => authorizationGroups.direct(user.id, params.organizationId, params.userId),
    });
    contract(app, {
        method: "POST",
        url: "/organizations/:organizationId/members/:userId/permission-grants",
        ...
        response: permissionGrant,
        status: 201,
        run: ({ user, params, body }) => authorizationGroups.grantDirect(user.id, params.organizationId, params.userId, body.permission),
    });
    contract(app, {
        method: "DELETE",
        ...
        response: z.null(),
        run: async ({ user, params }) => {
            await authorizationGroups.revokeDirect(user.id, params.organizationId, params.userId, params.permission);
            return null;
        },
    });
    contract(app, {
        ...
        status: 201,
        run: ({ user, params, body }) => invitations.issue(user.id, params.organizationId, body),
    });
    contract(app, {
        method: "POST",
    ...
    contract(app, {
        ...
        response: issuedInvitation,
        status: 201,
        run: ({ user, params }) => invitations.replace(user.id, params.organizationId, params.id),
    });
    contract(app, {
        method: "DELETE",
