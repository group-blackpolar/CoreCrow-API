...
function desktopToken(request) {
    const match = request.headers.authorization?.match(/^Bearer ([A-Za-z0-9._-]{20,256})$/);
    if (!match || match[1].startsWith("bp_"))
        fail(401, "UNAUTHENTICATED", "Desktop session required");
    return match[1];
}
export async function desktopAuthRoutes(app) {
    app.get("/google/start", {
        ...
        const response = await auth.handler(new Request(`${baseURL}/v1/auth/sign-in/social`, {
            method: "POST",
    ...
    app.get("/google/complete", {
        ...
        const code = await desktopIdentity.complete(pending, session.user.id, session.session.id);
        return reply.redirect(`${DESKTOP_CALLBACK}?code=${code}`);
    });
    app.get("/google/error", {
        schema: {
            tags: ["Identity"],
    ...
    app.post("/exchange", {
        ...
        schema: {
            ...
            response: {
                200: zodToJsonSchema(z.object({ token: z.string(), user: schemas.user }), { target: "openApi3" }),
            },
        },
    }, async (request) => {
