...
export const northComponentTypes = Object.freeze(Object.keys(catalog));
...
export const northComponentRegistry = Object.freeze(Object.fromEntries(northComponentTypes.map((type) => [`${type}@1`, {
        type,
        schemaVersion: 1,
        validate: (value) => catalog[type].safeParse(value),
        migrate: (value) => value,
    }])));
...
function validateRichNode(value, depth = 0) {
    ...
    if (node.type === "doc" && depth !== 0)
        fail(422, "RICH_TEXT_INVALID", "Nested document nodes are not allowed");
    if (node.type === "text") {
        if (typeof node.text !== "string" || node.text.length > 20_000)
            fail(422, "RICH_TEXT_INVALID", "Text node is invalid");
        if (node.content !== undefined)
            fail(422, "RICH_TEXT_INVALID", "Text nodes cannot contain child nodes");
    }
    if (node.href !== undefined && !safeUrl.safeParse(node.href).success)
        fail(422, "RICH_TEXT_INVALID", "Rich text link is invalid");
    if (node.assetId !== undefined && !resourceId.safeParse(node.assetId).success)
        fail(422, "RICH_TEXT_INVALID", "Rich text asset is invalid");
    ...
    if (node.content !== undefined) {
        if (!Array.isArray(node.content) || node.content.length > 2_000)
            fail(422, "RICH_TEXT_INVALID", "Rich text children are invalid");
        for (const child of node.content)
            validateRichNode(child, depth + 1);
    }
}
function assertNoExecutableContent(value) {
...
function collectAssetIds(type, props, output) {
    if (["image", "video", "file"].includes(type) && typeof props.assetId === "string")
        output.add(props.assetId);
    if (type === "card" && typeof props.assetId === "string")
        output.add(props.assetId);
    if (type === "rich_text") {
        const visit = (value) => {
            if (Array.isArray(value))
...
export async function validateNorthPanelDocument(value, context) {
    const parsed = northPanelDocumentInput.safeParse(value);
    if (!parsed.success)
        fail(422, "PANEL_DOCUMENT_INVALID", "Panel document schema is invalid");
    ...
    for (const section of document.sections) {
        if (sections.has(section.id))
            fail(422, "PANEL_DOCUMENT_INVALID", "Section IDs must be unique");
        sections.add(section.id);
        componentCount += section.components.length;
        for (const component of section.components) {
            if (components.has(component.id))
                fail(422, "PANEL_DOCUMENT_INVALID", "Component IDs must be unique");
            components.add(component.id);
            const key = `${component.type}@${component.schemaVersion}`;
            if (!Object.hasOwn(northComponentRegistry, key))
                fail(422, "COMPONENT_SCHEMA_UNKNOWN", "Component type or schema version is not registered");
            const type = component.type;
            const props = northComponentRegistry[key].validate(component.props);
            if (!props.success)
                fail(422, "COMPONENT_SCHEMA_INVALID", "Component props do not match the registered schema");
            if (type === "rich_text")
                for (const richDocument of Object.values(props.data.documents))
                    validateRichNode(richDocument);
            if (type === "embed" && !embedAllowed(props.data.url))
                fail(422, "EMBED_DOMAIN_NOT_ALLOWED", "Embed domain is not allowed");
            collectAssetIds(type, props.data, assets);
            bindingReferences.push(...Object.values(component.bindings));
        }
    }
    if (componentCount > limits.componentsPerPanel)
        fail(422, "PANEL_COMPONENT_LIMIT_EXCEEDED", `Panel cannot contain more than ${limits.componentsPerPanel} components`);
    if (assets.size) {
        if (!context.validateAssetReference)
            fail(422, "ASSET_VALIDATION_UNAVAILABLE", "Asset references cannot be validated");
    ...
    if (bindingReferences.length) {
        if (!context.validateBindingReference)
            fail(422, "BINDING_VALIDATION_UNAVAILABLE", "Binding references cannot be validated");
