...
function strings(value, output = []) {
    if (typeof value === "string")
        ...
        value.forEach((item) => strings(item, output));
    else if (value && typeof value === "object")
        Object.values(value).forEach((item) => strings(item, output));
    return output;
}
const matches = (needle, ...values) => values.flatMap((value) => strings(value)).some((value) => value.toLocaleLowerCase().includes(needle));
export const northSearch = {
    run(userId, organizationId, query) {
        return transaction(async (tx) => {
            ...
            const results = [];
            const wants = (kind) => !query.resourceType || query.resourceType === kind;
            const dateAllowed = (value) => !updatedAfter || value > updatedAfter;
            if (wants("CATEGORY") && (!query.status || ["ACTIVE", "ARCHIVED"].includes(query.status)) && !query.updatedBy) {
                ...
                for (const category of categories) {
                    if (!matches(needle, category.name, category.description))
                        continue;
                    if (!(await hasNorthCapability(tx, userId, "north.category.read", { organizationId, scope: "CATEGORY", categoryId: category.id })))
                        continue;
            ...
            if (wants("SUBCATEGORY") && (!query.status || ["ACTIVE", "ARCHIVED"].includes(query.status)) && !query.updatedBy) {
                ...
                for (const subcategory of subcategories) {
                    if (!matches(needle, subcategory.name, subcategory.description))
                        continue;
                    const target = { organizationId, scope: "SUBCATEGORY", categoryId: subcategory.categoryId, subcategoryId: subcategory.id };
                    if (!(await hasNorthCapability(tx, userId, "north.subcategory.read", target)))
                        continue;
            ...
            if (wants("PANEL") && (!query.status || ["DRAFT", "PUBLISHED", "ARCHIVED"].includes(query.status))) {
                ...
                for (const panel of panels) {
                    const target = { organizationId, scope: "PANEL", categoryId: panel.subcategory.categoryId, subcategoryId: panel.subcategoryId, panelId: panel.id };
                    ...
                    if (draftRequested) {
                        if (!revision || !(await hasNorthCapability(tx, userId, "north.panel.preview", target)))
                            continue;
                    }
                    else if (archivedRequested) {
                        if (!(await hasNorthCapability(tx, userId, "north.panel.read", target)))
                            continue;
                    }
                    else {
                        if (panel.status !== "PUBLISHED" || !revision)
                            continue;
                        if (!(await hasNorthCapability(tx, userId, "north.panel.read", target)))
                            continue;
                        if (!(await northPanelAudienceAllows(tx, userId, panel, panel.subcategory.categoryId)))
                            continue;
                    }
                    const updatedAt = revision?.createdAt ?? panel.updatedAt;
                    const updatedBy = revision?.createdBy ?? null;
                    if (!dateAllowed(updatedAt) || (query.updatedBy && updatedBy !== query.updatedBy))
                        continue;
                    if (!matches(needle, panel.name, panel.description, revision?.document))
                        continue;
            ...
            if (wants("ASSET") && !query.categoryId && (!query.status || query.status === "READY")) {
                ...
                if (canReadAssets)
                    for (const asset of assets) {
                        if (!matches(needle, asset.filename, asset.mime))
                            continue;
