...
import { northTemplateSnapshotInput } from "./template-schema.js";
...
async function requirePlatformTemplateManager(tx, actorId) {
...
function uniqueSlugs(values, code) {
    if (new Set(values).size !== values.length)
        fail(422, code, "Template sibling slugs must be unique");
}
async function validatedSnapshot(value, actorId) {
    const parsed = northTemplateSnapshotInput.safeParse(value);
    if (!parsed.success)
        fail(422, "TEMPLATE_INVALID", "Template snapshot is invalid");
...
function etag(id, panelId) {
    return `"${createHash("sha256").update(`${id}:${panelId}:1`).digest("base64url")}"`;
}
async function applySnapshot(tx, actorId, organizationId, templateId, version, snapshot, requestedSlug) {
    ...
    if (await repo.categoryBySlug(tx, organizationId, categorySlug))
...
export const northTemplates = {
    ...
    read(userId, organizationId, templateId, version) {
        return transaction(async (tx) => {
            await authorizeNorth(tx, userId, "north.template.read", { organizationId, scope: "ORGANIZATION" });
            const template = await tx.northTemplate.findFirst({ where: { id: templateId, status: "ACTIVE" } });
            if (!template)
                fail(404, "NOT_FOUND", "Template not found");
            ...
            if (!templateVersion)
                fail(404, "NOT_FOUND", "Template version not found");
            ...
            try {
                template = await tx.northTemplate.create({
                    data: {
                        ...
                        createdBy: actorId,
                        versions: { create: { version: 1, snapshot: snapshot, createdBy: actorId } },
                    },
                });
            }
            catch (error) {
    ...
    createVersion(actorId, templateId, snapshotInput) {
        return transaction(async (tx) => {
            await requirePlatformTemplateManager(tx, actorId);
            ...
            const created = await tx.northTemplateVersion.create({
                data: { templateId, version, snapshot: snapshot, createdBy: actorId },
            });
    ...
    update(actorId, templateId, input) {
        return transaction(async (tx) => {
            await requirePlatformTemplateManager(tx, actorId);
    ...
    apply(userId, organizationId, templateId, input) {
        return transaction(async (tx) => {
