...
export function normalizeNorthSlug(value) {
    return value.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLowerCase()
        .replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 60)
        .replace(/-+$/g, "");
}
export function validateNorthSlug(value, system = false) {
    const slug = normalizeNorthSlug(value);
    if (slug.length < 2)
        return { slug, error: "NORTH_SLUG_INVALID" };
    if (!system && reserved.has(slug))
        return { slug, error: "NORTH_SLUG_RESERVED" };
    return { slug };
}
