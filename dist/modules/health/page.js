...
function trafficChart(summary) {
    ...
    const bars = summary.series
        ...
        const measuredHeight = (bucket.requests / maximum) * (height - padding * 2);
  ...
  <main class="shell">
    ...
    <section class="details" aria-labelledby="details-title">
      ...
      <div class="dependency-list">${state.modules
        .map((module) => `<div><span>${moduleLabels[module.name] ?? module.name}</span><b class="pill ${module.status === "available" ? "" : "degraded"}">${module.status === "available" ? "Available" : "Unavailable"}</b></div>`)
        .join("")}</div>
