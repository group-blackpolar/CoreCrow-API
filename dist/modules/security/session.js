...
function enforceRequiredPasswordChange(request, user) {
    const path = request.url.split("?")[0] ?? "";
    if (user.passwordChangeRequired &&
        !((request.method === "GET" && path === "/v1/me") ||
            (request.method === "POST" &&
                path === "/v1/me/change-temporary-password")))
        fail(403, "PASSWORD_CHANGE_REQUIRED", "Change the temporary password before continuing");
}
function enforceActiveUser(user) {
