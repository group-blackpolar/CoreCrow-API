        ...
        await prisma.$transaction(async (tx) => {
            await tx.contactRequest.update({
                ...
                data: {
                    notificationStatus: "failed",
                    notificationClaimedAt: null,
                    notificationNextAttemptAt: attempt < maxAttempts
...
export function startContactNotificationWorker(reportError, intervalMilliseconds = 10_000) {
    let running = false;
    const tick = async () => {
        if (running)
            return;
        ...
        try {
            for (let processed = 0; processed < 10; processed += 1)
                if (!(await processOneContactNotification()))
                    break;
        }
        catch (error) {
            reportError(error);
