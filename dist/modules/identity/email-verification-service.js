...
function matchesHash(expected, candidate) {
    const left = Buffer.from(expected, "hex");
    const right = Buffer.from(candidate, "hex");
    return left.length === right.length && timingSafeEqual(left, right);
}
export async function issueEmailVerificationCode(emailInput) {
...
export async function requestEmailVerificationCode(emailInput) {
...
export async function confirmEmailVerificationCode(emailInput, code) {
    const email = normalizeEmail(emailInput);
    ...
    const result = await transaction(async (tx) => {
        ...
        if (!matchesHash(challenge.codeHash, candidateHash)) {
            ...
            await auditRepository.append(tx, {
                actorId: user.id,
                action: updated.attempts >= MAX_ATTEMPTS
