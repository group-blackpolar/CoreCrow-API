...
export async function bootstrapOperator(email, adminSecret, temporaryPassword) {
    ...
    return transaction(async (tx) => {
        ...
        if (accounts.length > 1)
            fail(409, "CREDENTIAL_STATE_INVALID", "The identity has multiple credential accounts");
        const otherSuperadmin = await credentials.otherSuperadmin(tx, user.id);
        if (otherSuperadmin && user.role !== "SUPERADMIN")
            fail(409, "ALREADY_INITIALIZED", "A different superadmin already exists; use authenticated role management");
        if (accounts[0])
            await credentials.updateCredential(tx, accounts[0].id, temporaryPasswordHash);
        else
