...
export async function legacyLogin(email, credential, ip) {
    if (!legacyEnabled())
        fail(410, "LEGACY_AUTH_DISABLED", "Use Better Auth at /v1/auth");
    email = email.trim().toLowerCase();
    const bucket = hash(email);
    const previous = attempts.get(bucket);
    if (previous && previous.until > Date.now() && previous.count >= 5)
        fail(429, "RATE_LIMITED", "Try again later");
    ...
    const result = await transaction(async (tx) => {
        ...
        if (!matches || !user?.adminUniqueId || !user.emailVerified || !["ADMIN", "SUPERADMIN"].includes(user.role))
...
export async function legacyPrincipal(token) {
    if (!legacyEnabled())
        fail(401, "UNAUTHENTICATED", "Use Better Auth");
    const session = await repo.session(hash(token));
    if (!session || session.expiresAt <= new Date() || !session.user.adminUniqueId || !["ADMIN", "SUPERADMIN"].includes(session.user.role))
        fail(401, "UNAUTHENTICATED", "Invalid legacy session");
    return { id: session.user.id, role: session.user.role };
}
export async function migrateLegacyIdentity(actorId, email, password) {
    if (!legacyEnabled())
        fail(410, "LEGACY_AUTH_DISABLED", "Legacy migration is disabled");
    const hashed = await hashPassword(password);
    return transaction(async (tx) => {
        const user = await repo.user(tx, email.toLowerCase());
        if (!user || user.id !== actorId || !user.adminUniqueId || !user.emailVerified)
            fail(403, "FORBIDDEN", "Identity cannot be migrated");
        if (await repo.credential(tx, actorId))
            fail(409, "CREDENTIAL_EXISTS", "Use the existing Better Auth credential");
